...
function stopLocalRace({clearParticipants=false}={}){
 ...
 sim=null;localRunning=false;localRaceConfig=null;raceHostId=null;lastRace=-1;lastWinnerRace=-1;winnerPopupFirstSeenAt=0;manualCam=null;
...
function connectRoomEvents(){
 ...
 eventSource.onmessage=(ev)=>{
  try{
   const incoming=JSON.parse(ev.data)?.state;if(!incoming)return;
...
function sendSnapshot(){if(!sim||snapshotInFlight||resetInFlight||mutationBusy)return;const epoch=lifecycleEpoch;snapshotInFlight=true;const activeSim=sim;apiQuiet('snapshot',{balls:activeSim.balls.filter(b=>!b.done||String(b.ballId)===String(activeSim.focusBallId||'')).map(b=>({ballId:b.ballId,name:b.name,copy:b.copy,x:+b.x.toFixed(1),y:+b.y.toFixed(1),done:!!b.done,qualified:!!b.qualified,rank:b.rank||0,vx:+b.vx.toFixed(2),vy:+b.vy.toFixed(2),stunMs:Math.max(0,Math.round(b.stunUntil-performance.now())),impactMs:Math.max(0,Math.round(Number(b.impactUntil||0)-performance.now())),waiting:!!b.waiting||performance.now()<(b.releaseAt||0)})),rot:activeSim.map.rot.map(r=>r.a),gate:activeSim.map.gate?activeSim.map.gate.a:0,cam:+activeSim.cam.toFixed(1),camX:+activeSim.camX.toFixed(1),camZoom:+activeSim.camZoom.toFixed(3)},2500).catch(()=>{}).finally(()=>{if(epoch===lifecycleEpoch)snapshotInFlight=false})}
...
function drawFrame(ts){const c=$('raceCanvas');if(!c)return;const sourceCount=(role==='admin'&&sim?sim.balls.length:(state?.snapshot?.balls||[]).length);const d=Math.min(devicePixelRatio||1,sourceCount>180?1:sourceCount>90?1.25:1.5),w=c.clientWidth,h=c.clientHeight;if(c.width!==Math.round(w*d)||c.height!==Math.round(h*d)){c.width=Math.round(w*d);c.height=Math.round(h*d)}const x=c.getContext('2d');x.setTransform(d,0,0,d,0,0);const theme=themes[state?.map||'wheel'];x.fillStyle=theme.bg;x.fillRect(0,0,w,h);let source=role==='admin'&&sim?sim.balls:(state?.snapshot?.balls||[]),map=role==='admin'&&sim?sim.map:mapDef(state?.map||'wheel',state?.seed||1);
 ...
 if(role==='admin'&&sim&&(localRunning||state?.status==='running')&&!sim.paused&&!document.hidden){
  ...
  const snapshotGap=sim.balls.length>800?320:sim.balls.length>500?260:sim.balls.length>250?190:120;if(ts-sim.lastSend>snapshotGap){sim.lastSend=ts;sendSnapshot()}
 }
...
function runBackgroundPhysics(){
 ...
 const snapshotGap=sim.balls.length>800?320:sim.balls.length>500?260:sim.balls.length>250?190:120;
...
function init(r){if(r==='unified')bindUnified();else{role=r;if(r==='admin'){loadAdminPrefs();bindAdmin();}if(r==='member')bindMember();}bindMinimapNavigation();connectRoomEvents();poll();requestAnimationFrame(draw)}return{init}})();
