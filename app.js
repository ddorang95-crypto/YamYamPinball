...
function normalizeSnapshot(snap){
 ...
 if(Array.isArray(snap.balls))snap={...snap,balls:snap.balls.map(b=>({...b,ownerInitial:ownerMark(b.owner||b.ownerInitial)}))};
 return snap;
}
...
function packSnapshotBalls(list){
 return list.map(b=>[b.ballId,b.name,b.copy,ownerMark(b.owner||b.ownerInitial),+b.x.toFixed(1),+b.y.toFixed(1),b.done?1:0,b.qualified?1:0,b.rank||0,+b.vx.toFixed(2),+b.vy.toFixed(2),(b.waiting||performance.now()<(b.releaseAt||0))?1:0]);
}
...
else if(incomingSeq)lastAcceptedSnapshotSeq=incomingSeq;
...
function bindAdmin(){
 ...
 $('startBtn').onclick=async()=>{const btn=$('startBtn');if(btn.disabled)return;resumeBlockedRaceId=0;sessionStorage.removeItem('pin_block_resume_'+room);localStartHardNotBefore=performance.now()+3000;showStartCountdown();const winnerCard=$('winnerCard');if(winnerCard)winnerCard.classList.remove('show','burst','winner-pop');winnerPopupFirstSeenAt=0;lastWinnerRace=-1;localWinnerLatch=null;const total=balls().length;if(!state||total<1){flash('공을 1개 이상 추가해주세요');return}const d=winDraft||{mode:state.winMode,ranks:state.winningRanks||[1]},map=selectedMapLock||$('mapSelect').value;btn.disabled=true;btn.textContent='동기화 중';try{const j=await apiQuiet('startRace',{map,winMode:d.mode,ranks:d.mode==='last'?[total]:d.ranks},6500);if(!j?.ok)throw Error(j?.error||'시작 오류');state={...state,map,status:'running',raceId:Number(j.raceId),seed:Number(j.seed),startedAt:Number(j.startedAt),raceHostId:String(j.raceHostId||clientId),winMode:j.winMode||d.mode,winningRanks:j.winningRanks||d.ranks,finishOrder:[],winners:[],winnerDeclared:false,snapshot:{balls:[],rot:[],cam:0}};raceHostId=state.raceHostId;snapshotSeq=0;lastAcceptedSnapshotSeq=0;scheduleSharedRace(state);ui();showStartCountdown();if(winDraft)winDraft.dirty=false}catch(e){flash('서버 동기화 오류: '+(e?.message||'통신 오류'))}finally{btn.disabled=false;btn.textContent='▶ 레이스 시작'}};$('resetBtn').onclick=async()=>{
  ...
  const keptParticipants=Array.isArray(state?.participants)?state.participants.map(p=>({...p})):[];
  try{
   ...
   clearWinnerPopupEverywhereLocal();
   if(j.state){
    if(keptParticipants.length&&(!Array.isArray(j.state.participants)||!j.state.participants.length))j.state.participants=keptParticipants;
    state=j.state;
   }
   winDraft={mode:state.winMode||'first',ranks:[...(state.winningRanks||[1])],dirty:false};
...
function ownerMark(value){const raw=String(value||'').trim(),v=raw.toLowerCase();if(v.includes('야미')||v==='얌'||v==='y'||v.includes('yami'))return'Y';if(v.includes('선하')||v==='선'||v==='s'||v==='m'||v.includes('seonha'))return'S';if(v.includes('도릿')||v==='도'||v==='d'||v.includes('dorit'))return'D';return raw.slice(0,1).toUpperCase()||'?'}
...
function balls(){const a=[];(state?.participants||[]).forEach(p=>{for(let i=1;i<=+p.count;i++)a.push({ballId:p.id+'_'+i,name:p.name,copy:i,owner:p.owner,ownerInitial:ownerMark(p.owner||p.ownerInitial)})});return a}
...
function drawFrame(ts){const c=$('raceCanvas');if(!c)return;const sourceCount=(isRaceHost()&&sim?sim.balls.length:((renderRemoteSnapshot||state?.snapshot)?.balls||[]).length);const d=Math.min(devicePixelRatio||1,sourceCount>180?1:sourceCount>90?1.25:1.5),w=c.clientWidth,h=c.clientHeight;if(c.width!==Math.round(w*d)||c.height!==Math.round(h*d)){c.width=Math.round(w*d);c.height=Math.round(h*d)}const x=c.getContext('2d');x.setTransform(d,0,0,d,0,0);const theme=themes[state?.map||'wheel'];x.fillStyle=theme.bg;x.fillRect(0,0,w,h);
 ...
 const mh=map.worldH||H,responsiveSideReserve=clamp(60+(w-900)*.6,60,250),renderBaseScale=Math.min((w-responsiveSideReserve)/W,1.02),bottomViewWorld=h/Math.max(.01,renderBaseScale*.86),bottomFixedCam=clamp((map.gate?.pivotY||map.finalZone?.gateY||mh-500)-bottomViewWorld*.49,0,Math.max(0,mh-bottomViewWorld+40));let active=source.filter(b=>!b.done),ys=active.map(b=>b.y).sort((a,b)=>a-b);
 ...
 const cam=isRaceHost()&&sim?sim.cam:targetCam,camX=isRaceHost()&&sim?sim.camX:Number((renderRemoteSnapshot||state?.snapshot)?.camX||W/2);let zoom=isRaceHost()&&sim?sim.camZoom:Number((renderRemoteSnapshot||state?.snapshot)?.camZoom||desiredZoom);
 ...
 x.save();x.translate(ox,-cam*sx);x.scale(sx,sx);drawMap(x,map,theme);map.rot.forEach((r,i)=>{const a=role==='admin'&&isRaceHost()&&sim?r.a:(state?.snapshot?.rot?.[i]||0),co=Math.cos(a),si=Math.sin(a);x.strokeStyle=theme.line;x.shadowColor=theme.glow;x.shadowBlur=18;x.lineWidth=16;x.beginPath();x.moveTo(r.x-co*r.len/2,r.y-si*r.len/2);x.lineTo(r.x+co*r.len/2,r.y+si*r.len/2);x.stroke();x.shadowBlur=0});if(map.gate){const a=role==='admin'&&isRaceHost()&&sim?map.gate.a:Number(state?.snapshot?.gate||0),co=Math.cos(a),si=Math.sin(a),half=map.gate.len/2;x.save();x.strokeStyle=theme.line;x.shadowColor=theme.glow;x.shadowBlur=22;x.lineWidth=18;x.lineCap='round';x.beginPath();x.moveTo(map.gate.pivotX-co*half,map.gate.pivotY-si*half);x.lineTo(map.gate.pivotX+co*half,map.gate.pivotY+si*half);x.stroke();x.fillStyle='#fff';x.beginPath();x.arc(map.gate.pivotX,map.gate.pivotY,11,0,Math.PI*2);x.fill();x.restore()}
 ...
 for(const b of source){if(b.done)continue;const alpha=(role==='admin'&&sim)?clamp(sim.acc/8.333,0,1):1,rx=(role==='admin'&&sim&&Number.isFinite(b.prevX))?b.prevX+(b.x-b.prevX)*alpha:b.x,ry=(role==='admin'&&sim&&Number.isFinite(b.prevY))?b.prevY+(b.y-b.prevY)*alpha:b.y;const hideAtWinnerEnd=!!(role==='admin'&&sim?.winnerResolved&&String(sim.focusBallId)===String(b.ballId)&&Number.isFinite(Number(b.winnerHideAt))&&performance.now()>=Number(b.winnerHideAt));if(hideAtWinnerEnd)continue;const focusStillLive=!!(winnerCinematic&&syncedFocusId&&String(syncedFocusId)===String(b.ballId)),isFocusBall=focusStillLive,cinematicActive=winnerCinematic;x.save();if(cinematicActive&&!isFocusBall)x.globalAlpha=.18;const pulse=isFocusBall?(1.62+Math.sin(performance.now()*.012)*.10):1,visualR=(b.r||R)*pulse;const hue=getNameHue(b.name),impactMs=role==='admin'&&sim?Math.max(0,Number(b.impactUntil||0)-performance.now()):Math.max(0,Number(b.impactMs||0)),impactLight=70+25*Math.min(1,impactMs/500);x.shadowColor=getNameColor(b.name,.95);x.shadowBlur=isFocusBall?34:(denseMode?0:10);x.fillStyle=`hsl(${hue} 86% ${Math.min(86,impactLight)}%)`;x.beginPath();x.arc(rx,ry,visualR,0,7);x.fill();x.lineWidth=isFocusBall?3:2;x.strokeStyle='#fff';x.stroke();x.shadowBlur=0;
  if(['GROUP','YAMYAM'].includes(String(room||'').toUpperCase())){const mark=String(ownerMark(b.owner||b.ownerInitial)||'?').slice(0,1);if(mark){x.textAlign='center';x.textBaseline='middle';x.font=`1000 ${Math.max(11,Math.round(visualR*1.05))}px Pretendard, Arial, sans-serif`;x.lineWidth=2.5;x.strokeStyle='rgba(0,0,0,.78)';x.strokeText(mark,rx,ry+1);x.fillStyle='#fff';x.fillText(mark,rx,ry+1)}}
 ...
 if(winnerCinematic&&focusWinnerBall){
  const a=(role==='admin'&&sim)?clamp(sim.acc/8.333,0,1):1;
  const wx=(role==='admin'&&Number.isFinite(focusWinnerBall.prevX))?focusWinnerBall.prevX+(focusWinnerBall.x-focusWinnerBall.prevX)*a:focusWinnerBall.x;
  const wy=(role==='admin'&&Number.isFinite(focusWinnerBall.prevY))?focusWinnerBall.prevY+(focusWinnerBall.y-focusWinnerBall.prevY)*a:focusWinnerBall.y;
  ...
  if(['GROUP','YAMYAM'].includes(String(room||'').toUpperCase())){const mark=String(ownerMark(focusWinnerBall.owner||focusWinnerBall.ownerInitial)||'?').slice(0,1);x.font='1000 26px Pretendard, Arial, sans-serif';x.lineWidth=5;x.strokeStyle='rgba(0,0,0,.82)';x.strokeText(mark,px,py+1);x.fillStyle='#fff';x.fillText(mark,px,py+1)}
...
function handleVisibilityContinuity(){
 const now=performance.now();
 if(document.hidden){if(state?.status==='running'){resumeBlockedRaceId=Number(state.raceId)||0;sessionStorage.setItem('pin_block_resume_'+room,String(resumeBlockedRaceId))}hiddenPhysicsLast=now;return}
 hiddenPhysicsLast=0;
